...
impl MessageHeader {
    fn init(dh_public_key: PublicKey, count: Counter, previous_count: Counter) -> Self {
        Self {
            dh_public_key,
            count,
            previous_count,
...
impl DoubleRatchet {
    ...
    pub fn initialise_alice(shared_root_key: KeySecret, bobby_public_key: PublicKey) -> Self {
    ...
    pub(crate) fn ratchet_encrypt(
        ...
        plaintext: &[u8],
        associated_data: &[u8],
    ) -> Result<(MessageHeader, Vec<u8>), DoubleRatchetError> {
    ...
    fn try_skipped_message_keys(
        ...
        match self
            ...
            .0
            .remove(&(header.dh_public_key, header.count))
        {
            Some(mk) => {
                let concat_ad = concat_header_and_ad(associated_data, header);
...
fn derive_hkdf_key_and_nonce(
    ...
    h.expand(HKDF_INFO_ENCRYPTION_KEY, &mut okm)
